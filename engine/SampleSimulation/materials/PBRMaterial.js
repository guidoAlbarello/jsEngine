class PBRMaterial extends Material {
  constructor() {
	...
	PBRMaterial.GLOBAL_UNIFORMS.push(Material.makeUniform("uPMatrix", gRenderer.MAT4, projectionMatrix));
    }
	...
	PBRMaterial.GLOBAL_UNIFORMS.push(Material.makeUniform("dirLight.direction", gRenderer.VEC3, light.direction));
	PBRMaterial.GLOBAL_UNIFORMS.push(Material.makeUniform("dirLight.intensity", gRenderer.VEC3, light.intensity));
    }

    static setPointLights(lights) {
	for (let i = 0; i < lights.length; i++) {
	    let light = lights[i];
	    PBRMaterial.GLOBAL_UNIFORMS.push(Material.makeUniform("pointLights["+i+"].position", gRenderer.VEC3, light.position()));
	    PBRMaterial.GLOBAL_UNIFORMS.push(Material.makeUniform("pointLights["+i+"].intensity", gRenderer.VEC3, light.intensity));
	}			  
    }

    static setCameraPosition(cameraPosition) {
    	PBRMaterial.GLOBAL_UNIFORMS.push(Material.makeUniform("cameraPos", gRenderer.VEC3, cameraPosition));
    }
	...
	this.addSampler("params.second_albedo", gTextureManager.getTexture(texture));
    }

    setRoughness(texture) {
	this.addSampler("params.roughness", gTextureManager.getTexture(texture));
    }
	...
	this.addSampler("params.ambient_occlusion", gTextureManager.getTexture(texture));
    }

    setNormalMap(texture) {
	this.addSampler("params.normal_map", gTextureManager.getTexture(texture));
    }

    setReflectionMap(texture) {
	this.addSampler("params.reflection_map", gTextureManager.getTexture(texture));
    }

    setReflectionIntensity(intensity) {
	this.addUniform("params.reflection_intensity", gRenderer.FLOAT, intensity);
    }
}
