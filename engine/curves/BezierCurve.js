class BezierCurve extends Curve {
	...
	if (controlPoints.length % 4 != 0) throw "ControlPoints arrays must be a multiple of 4";
	...
	for (let i = 0; i < controlPoints.length; i+=4) {
	    curveSections.push(new CubicBezierCurve(controlPoints[i], controlPoints[i+1], controlPoints[i+2], controlPoints[i+3]).setDefaultNormal(this.defaultNormal));
	}
