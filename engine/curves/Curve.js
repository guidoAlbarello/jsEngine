class Curve extends Object3d {
	...
	sample(t) {
		let sample =
			t == this.curveSections.length
				? this.curveSections[t - 1].curve(1.0)
				: this.curveSections[Math.floor(t)].curve(t - Math.floor(t));
		return sample;
	}

	point(t) {
		return t == this.curveSections.length
			? this.curveSections[t - 1].point(1.0)
	...
	getArcParametrization(samplesAmount) {
		...
		for (let i = 0; i < samplesAmount; i++) {
			let u = step * i;
			equidistantIndexes.push(
				this.findTFromDistance(
					(u * arcLength) / this.curveSections.length,
					indexToLength
				)
			);
		}
	...
	findTFromDistance(u, indexToLength) {
		...
		return (
			indexToLength[i - 1][1] +
			((indexToLength[i][1] - indexToLength[i - 1][1]) /
				(indexToLength[i][0] - indexToLength[i - 1][0])) *
				(u - indexToLength[i - 1][0])
		);
	}
