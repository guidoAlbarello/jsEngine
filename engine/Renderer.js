class Renderer {
	...
	if (!this.gl.getExtension('OES_element_index_uint')) alert("Browser does not support uint for indexBuffer length");
    }

    
    getTextureSampler(samplerNumber) {
	if (samplerNumber > this.gl.getParameter(this.gl.MAX_TEXTURE_IMAGE_UNITS)) alert("Amount of texture units not supported");
	...
	case 9: return this.gl.TEXTURE9;
	case 10: return this.gl.TEXTURE10;
	case 11: return this.gl.TEXTURE11;
	case 12: return this.gl.TEXTURE12;
	case 13: return this.gl.TEXTURE13;
	case 14: return this.gl.TEXTURE14;
	case 15: return this.gl.TEXTURE15;
	case 16: return this.gl.TEXTURE16;
	case 17: return this.gl.TEXTURE17;
	case 18: return this.gl.TEXTURE18;
	case 19: return this.gl.TEXTURE19;
	case 20: return this.gl.TEXTURE20;
	case 21: return this.gl.TEXTURE21;
	case 22: return this.gl.TEXTURE22;
	case 23: return this.gl.TEXTURE23;
	case 24: return this.gl.TEXTURE24;
	case 25: return this.gl.TEXTURE25;
	case 26: return this.gl.TEXTURE26;
	case 27: return this.gl.TEXTURE27;
	case 28: return this.gl.TEXTURE28;
	case 29: return this.gl.TEXTURE29;
	case 30: return this.gl.TEXTURE30;
	case 31: return this.gl.TEXTURE31;
	}
    }
    
    bindBuffer(valuesPerVertex, bufferLocation, shaderLocation, shaderProgram) {
        let attribute = this.gl.getAttribLocation(shaderProgram, shaderLocation);
	if (attribute != -1) {
            this.gl.enableVertexAttribArray(attribute);
            this.gl.bindBuffer(this.gl.ARRAY_BUFFER, bufferLocation);
            // TODO: See if it's useful to make this parameters dynamic. 
            this.gl.vertexAttribPointer(attribute, valuesPerVertex, this.gl.FLOAT, false, 0, 0);
	}
    }
	...
	// Maybe mesh isn't needed anymore
        this.elementsToDraw[material.Id()].push(new RenderData(mesh, material, modelMatrix));
    }
    ...
    drawScene() {
        ...
        for (let i = 0; i < this.elementsToDraw.length; i++) {      // Operations per shader
	    ...
	    if (this.elementsToDraw[i] == undefined || this.elementsToDraw[i].length == 0) continue;
	    ...
	    for (let i = 0; i < materialProgram.material.GLOBAL_UNIFORMS.length; i++) {
		let globalUniform = materialProgram.material.GLOBAL_UNIFORMS[i];
		this.bindUniform(globalUniform.name, globalUniform.type, globalUniform.data, materialProgram.program);
	    }
	    
            for (let j = 0; j < this.elementsToDraw[i].length; j++) {       // Operations per object
		...
		mat4.multiply(modelViewMatrix, this.viewMatrix, renderData.modelMatrix);
		...
		for (let k = 0; k < material.uniforms.length; k++) {
		    ...
		    this.bindUniform(uniform.name, uniform.type, uniform.data, materialProgram.program);
		}
		
		// Load per object attributes
		for (let k = 0; k < material.attributes.length; k++) {
		    let attribute = material.attributes[k];
                    this.bindBuffer(attribute.valuesPerVertex, attribute.location, attribute.name, materialProgram.program);
                }

		// Load per object textures
		for (let k = 0; k < material.textures.length; k++) {
		    let texture = material.textures[k];
		    this.bindTexture(texture.name, texture.data, materialProgram.program, texture.type);
		}
                
                this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, renderData.mesh.indexBufferLocation);	
                this.gl.drawElements(renderData.drawingMethod(), renderData.indexBufferLength(), this.gl.UNSIGNED_INT, 0);
