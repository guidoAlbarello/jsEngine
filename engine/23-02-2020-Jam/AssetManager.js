class AssetManager {
    ...
    loadData(modelData) {
		...
		mesh.attributes[attributes[i]["name"]] = [this.copyBufferToRenderer(attributes[i]["data"]), attributes[i]["data"]];     
        }

        mesh.indexBuffer = modelData.indexBuffer;
        mesh.drawingMethod = this.getWebglDrawingMethod(modelData.drawingMethod);
        
        mesh.indexBufferLocation = this.gl.createBuffer();
        this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, mesh.indexBufferLocation);
        this.gl.bufferData(this.gl.ELEMENT_ARRAY_BUFFER, new Uint32Array(mesh.indexBuffer), this.gl.STATIC_DRAW);
	...
	let tangents_temp = []; let tangents = [];
	...
	if (params.textureCoordinates.length > 0) {
		...
		let p1 = [params.positions[3*i1], params.positions[3*i1+1], params.positions[3*i1+2]];
		let p2 = [params.positions[3*(i2)], params.positions[3*(i2)+1], params.positions[3*(i2)+2]];
		let p3 = [params.positions[3*(i3)], params.positions[3*(i3)+1], params.positions[3*(i3)+2]];
		let uv1 = [params.textureCoordinates[2*i1], params.textureCoordinates[2*i1+1]];
		let uv2 = [params.textureCoordinates[2*i2], params.textureCoordinates[2*i2+1]];
		let uv3 = [params.textureCoordinates[2*i3], params.textureCoordinates[2*i3+1]];
		...
		let invDet = 1.0 / (deltaUV1[0] * deltaUV2[1] - deltaUV2[0] * deltaUV1[1]);
		...
		if (!(i1 == i2 || i1 == i3 || i3 == i2)) {
		    // P1
		    vec3.add(tangents_temp[i1][0], tangents_temp[i1][0], tangent);
		    tangents_temp[i1][1]++;

		    // P2
		    vec3.add(tangents_temp[i2][0], tangents_temp[i2][0], tangent);
		    tangents_temp[i2][1]++;
		    
		    // P3
		    vec3.add(tangents_temp[i3][0], tangents_temp[i3][0], tangent);
		    tangents_temp[i3][1]++;
		}
	    }
